...
impl GalenTool for WriteFile {
    ...
    async fn execute(&self, input: Value, ctx: &ToolContext) -> Result<String, String> {
        ...
        let result = tokio::task::spawn_blocking(move || {
            runtime::write_file(&target_str, &content_owned)
...
impl GalenTool for ReadFile {
    ...
    async fn execute(&self, input: Value, ctx: &ToolContext) -> Result<String, String> {
        ...
        let result = tokio::task::spawn_blocking(move || {
            runtime::read_file(&target_str, None, None)
        }).await.map_err(|e| format!("{e}"))?.map_err(|e| format!("{e}"))?;
...
impl GalenTool for ListFiles {
    ...
    async fn execute(&self, input: Value, ctx: &ToolContext) -> Result<String, String> {
        ...
        for entry in fs::read_dir(&target).map_err(|e| format!("{e}"))? {
            ...
            let ep = entry.path();
            let rel = ep.strip_prefix(&target).unwrap_or(&ep).to_string_lossy().to_string();
            entries.push(FileEntry {
                name, path: if sub.is_empty() { rel } else { format!("{}/{}", sub, rel) },
        ...
        let listing: Vec<String> = entries.iter().map(|e| {
            format!("{} {} ({} bytes)", if e.is_dir { "[DIR]" } else { "[FILE]" }, e.path, e.size)
        }).collect();
        Ok(if listing.is_empty() { format!("Empty: {}", if sub.is_empty() { "root" } else { &sub }) }
           else { format!("Contents:\n{}", listing.join("\n")) })
    }
}
...
impl GalenTool for SavePaper {
    ...
    async fn execute(&self, input: Value, ctx: &ToolContext) -> Result<String, String> {
        ...
        let paper = ctx.medical.fetch_article(pmid).await.map_err(|e| format!("{e}"))?
...
impl GalenTool for DeleteFile {
    ...
    fn is_write(&self) -> bool { true }
    async fn execute(&self, input: Value, ctx: &ToolContext) -> Result<String, String> {
        let target = resolve_workspace_path(&ctx.workspace_root, input["path"].as_str().ok_or("Missing 'path'")?)?;
...
impl GalenTool for MoveFile {
    ...
    fn is_write(&self) -> bool { true }
    async fn execute(&self, input: Value, ctx: &ToolContext) -> Result<String, String> {
        let from = resolve_workspace_path(&ctx.workspace_root, input["from"].as_str().ok_or("Missing 'from'")?)?;
        let to = resolve_workspace_path(&ctx.workspace_root, input["to"].as_str().ok_or("Missing 'to'")?)?;
        if let Some(parent) = to.parent() { fs::create_dir_all(parent).map_err(|e| format!("{e}"))?; }
