...
impl GalenTool for SearchFiles {
    fn definition(&self) -> ToolDefinition {
        ToolDefinition {
            ...
            description: Some("Search files by glob pattern in the workspace. Supports optional grep filtering.".into()),
            input_schema: json!({
                "type": "object",
                "properties": {
    ...
    async fn execute(&self, input: Value, ctx: &ToolContext) -> Result<String, String> {
        ...
        let target = resolve_workspace_path(&ctx.workspace_root, input["path"].as_str().unwrap_or(""))?;
        ...
        for entry in paths.flatten() {
            let rel = entry.strip_prefix(&target).unwrap_or(&entry).to_string_lossy().to_string();
            let meta = fs::metadata(&entry).ok();
            let prefix = if meta.as_ref().map(|m| m.is_dir()).unwrap_or(false) { "[DIR]" } else { "[FILE]" };
            if let Some(g) = grep {
                if !meta.as_ref().map(|m| m.is_dir()).unwrap_or(true) {
                    if let Ok(content) = fs::read_to_string(&entry) {
                        if content.contains(g) {
                            let preview = content.lines().filter(|l| l.contains(g)).take(5).collect::<Vec<_>>().join("\n  ");
                            results.push(format!("{prefix} {rel}\n  {preview}"));
                        }
                    }
                }
            } else {
                results.push(format!("{prefix} {rel} ({} bytes)", meta.map(|m| m.len()).unwrap_or(0)));
            }
        }
