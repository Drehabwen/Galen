...
impl StreamState {
    ...
    fn ingest_chunk(&mut self, chunk: ChatCompletionChunk) -> Result<Vec<StreamEvent>, ApiError> {
        ...
        for choice in chunk.choices {
            // Reasoning models (DeepSeek V4, etc.) emit chain-of-thought in
            // `reasoning_content` and the final answer in `content`.  Keep them
...
pub fn build_chat_completion_request(
    ...
    // thinking config for DeepSeek V4 (e.g. `{"type": "enabled"}`)
    if let Some(thinking) = &request.thinking {
        payload["thinking"] = json!({ "type": thinking.thinking_type });
    }
