...
You can add custom aliases in any settings file (`$CLAW_CONFIG_HOME/settings.json`, `~/.claw/settings.json`, `.claw/settings.json`, or `.claw/settings.local.json`):
...
1. `~/.claw.json`
2. `$CLAW_CONFIG_HOME/settings.json` (defaults to `~/.claw/settings.json`)
3. `<repo>/.claw.json`
